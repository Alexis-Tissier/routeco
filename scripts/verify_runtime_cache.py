...
def _request_payload(
    *,
    fuel_price: float,
    toll_estimate_rate: float = 0.105,
) -> dict:
...
def _health(client: httpx.Client, api_url: str) -> dict:
...
def verify(client: httpx.Client, api_url: str) -> None:
    ...
    first_response = client.post(
        endpoint,
        json=_request_payload(fuel_price=1.82),
    )
    ...
    health_after_first = _health(client, api_url)
    toll_after_first = health_after_first.get("toll_quote_cache") or {}

    second_started = time.perf_counter()
    second_response = client.post(
        endpoint,
        json=_request_payload(fuel_price=2.10),
    )
    ...
    immutable = ("id", "duration_minutes", "distance_km", "geometry", "toll_cost")
    changed = [
        field for field in immutable if first_fastest.get(field) != second_fastest.get(field)
    ]
    if changed:
        raise RuntimeError(
            "Le recalcul économique a modifié le tracé ou son péage : " + ", ".join(changed) + "."
        )
    ...
    health_after_second = _health(client, api_url)
    routing_cache = health_after_second.get("routing_cache") or {}
    toll_cache = health_after_second.get("toll_quote_cache") or {}
    if int(routing_cache.get("hits") or 0) < 1:
        raise RuntimeError("Le cache de tracés n'a enregistré aucune réutilisation.")

    toll_hits_delta = int(toll_cache.get("hits") or 0) - int(toll_after_first.get("hits") or 0)
    if toll_hits_delta < int(second.get("candidate_count") or 1):
        raise RuntimeError(
            "Le recalcul a relancé une partie du moteur de péage au lieu de "
            "réutiliser chaque tarification."
        )
    if second_wall > 2.5:
        raise RuntimeError(f"Le recalcul complet reste trop lent : {second_wall:.3f} s.")

    print(
        "Caches Routeco validés : "
        f"{first.get('candidate_count', 0)} candidats ; "
        f"première réponse complète {first_wall:.2f} s ; "
        f"recalcul carburant complet {second_wall:.3f} s ; "
        f"{toll_hits_delta} péage(s) réutilisé(s) ; "
        f"{int(routing_cache.get('estimated_bytes') or 0) / 1024 / 1024:.1f} Mio."
    )


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Vérifie les caches de tracés et de péages Routeco."
    )
    parser.add_argument(
        "--api-url",
